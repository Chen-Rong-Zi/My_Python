...
def get_images(html):
    ...
    # 提取下一页的url
    rule     = '21</a><a href="(.*?)">下一页»</a>'
    NextPage = re.findall(rule, html)
    NextPage_link = '''http://www.k2r2.com''' + NextPage[0]
...
def save_images(url_list,page=1):
    ...
    # 封装请求，头部信息
        ...
        with open(f_name, 'wb+') as f_name:
            f_name.write(image.read())



# 递归结构
def continue_get(NextPage_link,times=0):
    url     = NextPage_link
    #包装头部信息
    request = urllib.request.Request(url,headers=head)
    # 获取html
    ...
    # 保存图片
    try:
        save_images(url_list, times + 1)
        print('保存网页成功\n')
    except:
        print('保存网页失败\n')
    print(f'这一页:{url}')
    print(f'下一页:{Nextpage_link}\n')
    try:
...
'''爬虫程序完结撒花，从2022/9/24 8点至2022/9/25 14点，间断地完成了该程序。复习了BeautifuSoup的使用和文件的打开写入操作。期间学习了递归函数，利用其循环的特点，解决了爬虫爬取下一页面的递归过程。期间在写入文件名时，为了表示第一页、第二页......遇到困难，百思不得其解。原因是我尝试用函数被循环的次数（即是类似for循环中的i的性质）表示页码，但因为利用递归来做到连续多次爬取，无法将for循环融合进去。最终是利用递归函数条件变量（times）和页码间的关系完成。'''
